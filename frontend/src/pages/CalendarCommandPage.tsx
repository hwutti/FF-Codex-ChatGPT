...
const calendarApi = {
  getCategories: () => api.get('/calendar-command/categories').then(r => r.data),
  createCategory: (data: any) => api.post('/calendar-command/categories', data).then(r => r.data),
  updateCategory: (id: string, data: any) => api.put(`/calendar-command/categories/${id}`, data).then(r => r.data),
  deleteCategory: (id: string) => api.delete(`/calendar-command/categories/${id}`),
  getEvents: (from: string, to: string) => api.get(`/calendar-command/events?from=${from}&to=${to}`).then(r => r.data),
  createEvent: (data: any) => api.post('/calendar-command/events', data).then(r => r.data),
  updateEvent: (id: string, data: any) => api.put(`/calendar-command/events/${id}`, data).then(r => r.data),
  deleteEvent: (id: string) => api.delete(`/calendar-command/events/${id}`),
};
...
const PRESET_COLORS = ['#1e40af','#3b82f6','#10b981','#f59e0b','#8b5cf6','#ec4899','#06b6d4','#f97316'];
...
// ── Main Calendar Page ────────────────────────────────────────────────────────
export default function CalendarCommandPage() {
  const { user } = useAuth();
  ...
  const feedUrl = `${window.location.origin}/api/calendar-command/feed.ics`;
  ...
  const catColor = (ev: any) => ev.category?.color || '#1e40af';
                      ...
                      return (
                        <div key={i}
                          ...
                          className={`relative text-center text-[10px] py-0.5 rounded cursor-pointer transition-colors ${
                            !inMonth ? 'opacity-0' :
                            isT ? 'bg-blue-700 text-white font-bold' :
          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i}
                ...
                onClick={() => { if (isAdmin) { routerNavigate(`/calendar-command/new?date=${format(d, 'yyyy-MM-dd')}`); } }}>
                <div className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-medium mb-0.5 ${
                  isT ? 'bg-blue-700 text-white' :
          ...
          {days.map((d, i) => (
            <div key={i} className="text-center py-3 border-r border-surface-100 last:border-0">
              ...
              <p className={`text-lg font-bold mt-0.5 w-8 h-8 mx-auto flex items-center justify-center rounded-full ${
                isToday(d) ? 'bg-blue-700 text-white' : 'text-gray-900'
              }`}>{format(d, 'd')}</p>
            </div>
          ))}
        </div>
    ...
    <div className="flex flex-col h-full -m-4 sm:-m-6 lg:-m-8">
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-white border-b border-surface-200 flex-shrink-0 flex-wrap">
        ...
        <div className="flex items-center gap-1">
          {isAdmin && (
            <>
              <button onClick={() => routerNavigate('/calendar-command/categories')}
                className="p-2 text-gray-500 hover:bg-surface-100 rounded-xl transition-colors" title="Kategorien">
                <Tag className="w-4 h-4" />
              </button>
              <button onClick={() => routerNavigate(`/calendar-command/new?date=${currentDate.toISOString().slice(0,10)}`)}
                className="flex items-center gap-1.5 text-sm py-1.5 px-3 rounded-xl bg-blue-700 hover:bg-blue-800 text-white font-semibold transition-colors">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors"
            title={feedUrl}>
      ...
      {loading ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="w-7 h-7 border-2 border-surface-200 border-t-blue-700 rounded-full animate-spin" />
        </div>
      ) : (
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-modal w-full sm:max-w-md">
            ...
            {isAdmin && (
              <div className="flex gap-2 px-5 pb-5">
                <button onClick={() => { routerNavigate(`/calendar-command/${selectedEvent.id}/edit`); setSelectedEvent(null); }}
