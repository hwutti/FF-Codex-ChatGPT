...
// ── Storage key ───────────────────────────────────────────────────────────────
const STORAGE_KEY = 'fw_document_categories';

const DEFAULT_CATEGORIES = [
  { id: 'descriptions', label: 'Beschreibungen', icon: 'ScrollText',     color: '#3b82f6', description: 'Dienstvorschriften, Beschreibungen und Regelwerke' },
  { id: 'templates',    label: 'Vorlagen',        icon: 'FileText',       color: '#8b5cf6', description: 'Formulare, Vorlagen und Musterdokumente' },
  { id: 'training',     label: 'Schulungsunterlagen', icon: 'GraduationCap', color: '#10b981', description: 'Ausbildungsunterlagen und Lernmaterial' },
];
...
export default function DocumentsPage() {
  ...
  const load = () => {
    setLoading(true);
    documentApi.list({ isPublic: false }).then(data => {
      setDocs(data || []);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight" style={{ fontFamily: 'var(--font-headings)' }}>Dokumente Kommando</h1>
          <p className="text-sm text-gray-500 mt-0.5">Interne Unterlagen, Vorlagen und Schulungsmaterial</p>
        </div>
              ...
              <div key={cat.id} className="card p-0 overflow-hidden">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-1 px-4 pb-3" onClick={e => e.stopPropagation()}>
                    <button onClick={() => navigate(`/documents/edit?edit=${cat.id}`)}
                    ...
                    <button onClick={() => navigate(`/documents/upload?category=${cat.id}&label=${encodeURIComponent(cat.label)}&back=/documents`)}
                      ...
                      {catDocs.map((doc: any) => (
                        <div key={doc.id} className="flex items-center gap-3 px-5 py-3 border-b border-surface-100 last:border-0 hover:bg-surface-50 transition-colors">
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0 flex-wrap">
                            <button onClick={() => navigate(`/documents/${doc.id}?back=/documents`)}
