...
import { Upload, X, ArrowLeft, FileText, CheckCircle } from 'lucide-react';
...
export default function ProtocolUploadPage() {
  ...
  useEffect(() => {
    eventApi.list().then(e => setEvents(Array.isArray(e) ? e : (e.events || [])));
  }, []);
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900" style={{ fontFamily: 'var(--font-headings)' }}>Protokoll hochladen</h1>
          <p className="text-sm text-gray-500">PDF, Word oder OpenDocument — max. 50MB</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass-card rounded-2xl border border-black/10  p-6 space-y-5">
          <div>
            ...
            <input type="text" className="input-field" value={form.title}
              ...
              placeholder="z.B. Protokoll Monatsversammlung Jänner 2026" required />
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Verknüpftes Ereignis (optional)</label>
            <select className="input-field" value={form.eventId} onChange={e => setForm(f => ({ ...f, eventId: e.target.value }))}>
              ...
              {events.map((ev: any) => (
                <option key={ev.id} value={ev.id}>
                  {format(parseISO(ev.date), 'd. MMM yyyy', { locale: de })} · {ev.title}
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="flex gap-3">
          <button type="button" onClick={() => navigate('/protocols')} className="btn-secondary flex-1">
            Abbrechen
          </button>
          <button type="submit" disabled={uploading || !file} className="btn-primary flex-1 flex items-center justify-center gap-2">
            {uploading
              ...
              : <><Upload className="w-4 h-4" /> Protokoll hochladen</>
            }
          </button>
        </div>
      </form>
